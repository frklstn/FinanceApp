...
export function Modal({ isOpen, onClose, title, children }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      {/* Backdrop */}
      <div
        className="absolute inset-0 bg-black/60 dark:bg-black/90 backdrop-blur-xl transition-opacity duration-250 ease-out cursor-pointer"
        onClick={onClose}
      />
      ...
      <div className="nexus-popup w-full max-w-5xl overflow-hidden text-text-primary relative animate-in fade-in zoom-in-95 duration-250 ease-out rounded-3xl border border-border/40 shadow-2xl">
        ...
        <div className="flex justify-between items-center px-8 py-6 border-b border-border/20 bg-black/[0.02] dark:bg-white/[0.02]">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl bg-border/20 hover:bg-border/40 text-text-muted hover:text-text-primary transition-[background-color,color,transform] duration-160 ease-out active:scale-95 cursor-pointer border border-border/20"
          >
            <svg
