    ...
    <div className="pinjol-insight-card mb-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
        <p className="pinjol-insight-label mb-0">
          ...
          {t('forecast.survivalQuestion', 'Can I Survive?')}
        </p>
        <Button size="sm" onClick={onAnalyze} loading={loading} className="cursor-pointer">
          {t('forecast.action.analyze', 'Analyze My Situation')}
        </Button>
      </div>
      {insight ? (
        ...
        <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary">
          {t('forecast.analysisDesc', 'Klik tombol di atas untuk analisis deterministik berdasarkan forecast dan data utang aktif Anda.')}
        </p>
      )}
    </div>
  );
