  ...
  const fetchAllUsers = useCallback(async () => {
    try {
      const { data: profiles, error } = await supabase
        .from('profiles')
        .select('id, email, full_name, avatar_url, is_suspended, created_at')
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-light-text-secondary dark:text-dark-text-secondary mt-0.5">
            Admin Portal to manage user profiles, change avatar logos, and toggle account suspensions.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-light-border/30 dark:divide-dark-border/30 text-xs font-semibold">
              {filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-5 py-12 text-center text-light-text-secondary dark:text-dark-text-secondary font-medium">
                    No registered users matched your current query filter.
                  </td>
                </tr>
              ) : (
                filteredUsers.map((user) => (
                  <tr
                    ...
                    className={`hover:bg-light-bg/20 dark:hover:bg-dark-bg/10 transition-colors duration-150 ${
                      user.is_suspended ? 'opacity-70 bg-danger/5 dark:bg-danger/10' : ''
                    }`}
                  >
                    {/* User Profile */}
                    ...
                    <td className="px-5 py-3.5">
                      ...
                      >
                        {user.is_suspended ? <Lock className="w-3 h-3" /> : <Unlock className="w-3 h-3" />}
                    ...
                    <td className="px-5 py-3.5 text-right space-x-2">
                      ...
                      <button
                        ...
                        title={user.is_suspended ? 'Enable User Account' : 'Suspend User Account'}
                      >
              ...
              <div className="space-y-1.5">
                ...
                {editAvatar && (
                  <div className="flex items-center gap-2.5 pt-2">
                    <span className="text-[10px] text-light-text-secondary">Preview Logo:</span>
                    <div className="w-8 h-8 rounded-full border border-light-border/40 dark:border-dark-border/40 bg-light-bg dark:bg-dark-bg overflow-hidden flex items-center justify-center">
                      <img src={editAvatar} alt="Logo preview" className="w-full h-full object-cover" />
                    </div>
                  </div>
                )}
              </div>
