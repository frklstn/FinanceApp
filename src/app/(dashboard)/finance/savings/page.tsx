  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          [1, 2, 3].map((n) => <div key={n} className="h-64 rounded-2xl border border-line bg-surface animate-pulse" />)
        ) : goals.length === 0 ? (
          <EmptyState
                  ...
                  <Card className="h-full border-line bg-surface transition-all flex flex-col justify-between">
                    <div className="space-y-6">
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          <h4 className="text-lg font-semibold text-text-primary tracking-tight">{goal.name}</h4>
                        ...
                        <button onClick={() => handleDelete(goal.id)} className="p-2.5 rounded-xl bg-surface hover:bg-rose-500/20 text-text-muted hover:text-text-primary transition-all cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-line">
                      {finished ? (
                        ...
                        <Button
                          variant="outline"
                          className="w-full rounded-2xl border-line bg-surface text-xs py-6"
      ...
      <Modal isOpen={isGoalModalOpen} onClose={() => setIsGoalModalOpen(false)} title="Buat target baru">
        <form onSubmit={handleCreateGoal} className="space-y-6">
          ...
          <Button type="submit" variant="primary" loading={submitting} className="w-full h-12 rounded-2xl">Simpan target</Button>
        </form>
      </Modal>
      ...
      <Modal isOpen={isContributionModalOpen} onClose={() => setIsContributionModalOpen(false)} title={`Alokasikan: ${selectedGoal?.name ?? ''}`}>
        <form onSubmit={handleContributionSubmit} className="space-y-6">
          ...
          <Button type="submit" variant="primary" loading={submitting} className="w-full h-12 rounded-2xl">Alokasikan</Button>
        </form>
      </Modal>
    </div>
  );
