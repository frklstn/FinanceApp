...
export default function DashboardPage() {
  ...
  const HeroWidgets = [
    { label: t('dashboard.hero.netWorth', 'Total aset bersih (gabungan dompet)'), value: financialStats.totalBalance, icon: Wallet, color: 'text-primary', path: '/finance/wallets' },
    { label: t('dashboard.widget.income', 'Pemasukan'), value: financialStats.income, icon: TrendingUp, color: 'text-primary', action: () => setQuickAdd({ open: true, type: 'income' }) },
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {HeroWidgets.map((w, i) => (
          <motion.button
            ...
            className="app-card relative group p-4 md:p-6 text-left overflow-hidden cursor-pointer min-h-[120px] md:min-h-[140px] flex flex-col justify-between"
          >
            ...
            <div className="space-y-1">
              ...
              <div className="text-lg md:text-2xl font-semibold text-text-primary tracking-tight truncate">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        <Card className="md:col-span-2">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-heading text-base font-semibold text-text-primary flex items-center gap-2.5">
              <Zap className="w-4 h-4 text-primary" /> {t('dashboard.chart.expenseTrendsTitle', 'Grafik pengeluaran')}
            </h3>
          </div>
        ...
        <Card className="flex flex-col justify-between min-h-[320px]">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="w-10 h-10 rounded-2xl bg-primary-glow border border-primary-border flex items-center justify-center">
                <ShieldCheck className="w-5 h-5 text-primary" />
              </div>
              <div className="font-heading text-3xl font-semibold text-text-primary tracking-tight">{financialStats.score}</div>
            </div>

            <div className="space-y-1.5">
              <div className="flex justify-between text-xs text-text-secondary">
                <span>{t('dashboard.hero.scoreLabel', 'Skor kesehatan')}</span>
                <span className={financialStats.score > 70 ? "text-success" : "text-amber-600 dark:text-amber-400"}>
              ...
              <div className="h-1.5 w-full bg-surface rounded-full overflow-hidden">
                <motion.div
                  ...
                  className={`h-full ${financialStats.score > 70 ? "bg-success" : "bg-amber-500"}`}
                />
              </div>
            </div>
            ...
            <div className="pt-2 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2.5 rounded-xl bg-surface border border-line">
                  ...
                  <p className={`text-xs font-semibold ${financialStats.savingsDiff >= 0 ? 'text-success' : 'text-rose-600 dark:text-rose-400'}`}>
                ...
                <div className="p-2.5 rounded-xl bg-surface border border-line">
                  ...
                  <p className={`text-xs font-semibold ${financialStats.activeLoansCount > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-success'}`}>
          ...
          <Button variant="primary" className="w-full mt-4 py-3 h-auto text-xs" onClick={() => router.push('/finance/insights')}>
          ...
          <div className="space-y-2 flex-1">
            {financialStats.activeLoans.length > 0 ? (
              financialStats.activeLoans.slice(0, 2).map((loan) => (
                <div key={loan.id} className="p-3.5 rounded-2xl bg-surface border border-line flex items-center justify-between">
                  <div className="space-y-0.5">
                    <p className="text-xs font-medium text-text-primary">{loan.app_name || 'Pinjol'}</p>
                    <p className="text-[10px] text-rose-600/70 dark:text-rose-400/70">{loan.category}</p>
                  </div>
                  <p className="text-xs font-semibold text-text-primary">{formatCurrency(Number(loan.monthly_payment))}</p>
                </div>
              ))
            ) : (
              <div className="p-6 text-center rounded-2xl bg-surface border border-line">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="flex flex-col h-full gap-4">
          ...
          {categoryBreakdown.length === 0 ? (
            <div className="p-6 text-center rounded-2xl bg-surface border border-line flex-1 flex items-center justify-center">
              <p className="text-xs text-text-muted">
            ...
            <>
              <div className="space-y-1.5">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-lg font-semibold text-primary">
                    ...
                    <span className="text-xs font-normal text-text-muted">{categoryBreakdown[0].name}</span>
                  </span>
                </div>
                <div className="h-4 w-full bg-surface rounded-full overflow-hidden flex gap-0.5 p-0.5 border border-line">
                  {categoryBreakdown.map((cat, i) => (
                    <div
                      key={cat.name}
                      className={`h-full rounded-full ${['bg-primary', 'bg-rose-500/70', 'bg-amber-500/70', 'bg-text-muted'][i]}`}
              ...
              <div className="grid grid-cols-2 gap-3 flex-1">
                {categoryBreakdown.map((cat, i) => (
                  <div key={cat.name} className="p-3.5 rounded-2xl bg-surface border border-line flex flex-col justify-center">
                    <p className="text-[10px] text-text-secondary truncate">{cat.name}</p>
                    <p className={`text-sm font-semibold ${['text-primary', 'text-rose-600 dark:text-rose-400', 'text-amber-600 dark:text-amber-400', 'text-text-secondary'][i]}`}>
          ...
          <div className="space-y-3 flex-1">
            ...
            {recentTxs.slice(0, 3).map((tx) => (
              <button
                ...
                className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-surface border border-line hover:border-primary-border transition-colors cursor-pointer text-left"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${tx.type === 'income' ? 'bg-success/10 text-success' : 'bg-rose-500/10 text-rose-600 dark:text-rose-400'}`}>
                    {tx.type === 'income' ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
                  </div>
                  <span className="text-xs font-medium text-text-primary truncate">{tx.note || '-'}</span>
                </div>
                <span className={`text-xs font-semibold shrink-0 ${tx.type === 'income' ? 'text-success' : 'text-rose-600 dark:text-rose-400'}`}>
