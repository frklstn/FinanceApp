...
export default function BudgetsPage() {
  ...
  const fetchBudgets = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Gagal memuat anggaran belanja.';
      toast(msg, 'danger');
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveBudget = async (e: React.FormEvent) => {
    ...
    if (isNaN(amountNum) || amountNum <= 0) {
      toast('Masukkan batas pengeluaran yang valid.', 'danger');
      return;
    }
    ...
    try {
      ...
      toast('Batas anggaran berhasil disimpan!', 'success');
      setIsModalOpen(false);
      setCategoryId('');
      setLimitAmount('');
      fetchBudgets();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Gagal menyimpan target anggaran.';
      toast(msg, 'danger');
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDeleteBudget = async (id: string) => {
    if (!confirm('Apakah Anda yakin ingin menghapus batas anggaran kategori ini?')) return;
    try {
      await budgetService.deleteBudget(id);
      toast('Batas anggaran berhasil dihapus.', 'success');
      fetchBudgets();
    } catch (err: unknown) {
