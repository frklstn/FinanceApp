...
| **🛡️ Admin Portal** | Panel kustomisasi nama aplikasi, logo, dan judul tab browser secara global untuk seluruh pengguna. | Khusus Admin |
...
graph TD
    ...
    O -- Ya --> P[Admin Portal]
    P --> Q[Edit Global Branding: Nama, Logo, Tab Browser]
    Q --> R[Update Tabel app_settings di Supabase]
    R --> S[Perubahan Tampil Instan di Semua Client]
    O -- Tidak --> E
```
...
1.  `001_initial_schema.sql` (Skema Tabel Pengguna, Dompet, Transaksi, Anggaran, Tabungan, Utang)
2.  `002_loan_tracker.sql` (Skema Tabel Pelacak Pinjaman Online)
3.  `003_debt_survival_planner.sql` (Skema Tabel Proyeksi Pemasukan & Hari Gajian)
4.  `004_app_settings.sql` (Skema Tabel Konfigurasi Branding Global)
...
4.  **Siapkan Pinjol Tracker:**
    ...
    *   Tentukan tanggal gajian rutin Anda (misalnya tanggal 25) pada panel proyeksi pendapatan, serta masukkan estimasi gaji bersih Anda.
